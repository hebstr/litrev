...
allowed-tools: Read Write Edit WebFetch mcp__litrev-mcp__fetch_abstracts
---
...
**What Step 3 can do:** re-examine articles using title, metadata (study type, journal, year), and any available abstract. Do NOT fetch full-text PDFs — that is downstream (`litrev-extract` / verification phase). Step 3 adds value by: (a) catching no-abstract articles that are clearly out of scope based on metadata, (b) identifying duplicate cohorts among retained articles, (c) applying stricter scrutiny to borderline cases. If no additional information is available beyond what was already screened, retain the article and note it in the log.
